// Get your shorts on - this is an array workout!
    ...
    const people = ['Beck, Glenn', 'Becker, Carl', 
                    ...
                    'Bellow, Saul', 'Benchley, Robert', 'Benenson, Peter', 
                    'Ben-Gurion, David', 'Benjamin, Walter', 'Benn, Tony', 
                    'Bennington, Chester', 'Benson, Leana', 'Bent, Silas', 
                    'Bentsen, Lloyd', 'Berger, Ric', 'Bergman, Ingmar', 
    ...
    const oderedBirth = inventors.sort(function(olderInventor, youngerInventor) {
        
        if(olderInventor.year < youngerInventor.year) {
            return -1;
        }
        else {
            return 1;
    ...
    console.table(oderedBirth);

    // Array.prototype.reduce()
    // 4. How many years did all the inventors live?
    const yearsAlive = inventors.reduce((total, inventor) => {
    ...
    const transportation = data.reduce(function(object, items) {
        if(!object[items]) {
            object[items] = 0;
        }
        object[items]++;
